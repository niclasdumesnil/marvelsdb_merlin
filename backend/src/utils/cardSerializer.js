...
// Path to web/bundles/cards on the Symfony side (for image resolution).
// Adjust if the Node server doesn't serve static files from there.
const CARDS_DIR = path.resolve(__dirname, '../../..', 'web', 'bundles', 'cards');

/**
 * Resolve the best available image src for a card code.
 * Checks EN/FR webp, png, jpg, and 'a' variants, mirroring PHP logic.
 */
function resolveImage(code, suffix = '', lang = 'EN') {
  const fileName = `${code}${suffix}`;
  const candidates = [
    `${lang}/${fileName}.webp`,
    `${lang}/${fileName}.png`,
    `${lang}/${fileName}.jpg`,
    `${lang}/${fileName}a.webp`,
    `${lang}/${fileName}a.png`,
    `${lang}/${fileName}a.jpg`,
    `${fileName}.webp`,
    `${fileName}.png`,
  ...
  for (const c of candidates) {
    try {
      if (fs.existsSync(path.join(CARDS_DIR, c))) {
        return `/bundles/cards/${c}`;
      }
    } catch {
      // ignore fs errors
    }
