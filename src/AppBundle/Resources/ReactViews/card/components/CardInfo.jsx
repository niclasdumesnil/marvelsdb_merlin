...
/**
 * Renders card type, traits, and stat properties.
 */
export default function CardInfo({ card, showSpoilers, showType = true }) {
  ...
  return (
    <div className={spoilerClass}>
      ...
      {/* Traits - display as bold uppercase (e.g. "SKILL. THWART.") */}
      {card.traits && (
...
function CardProps({ card }) {
  ...
  // Upgrade / Event / Support / Ally
  if (['upgrade', 'event', 'support', 'ally'].includes(t)) {
    propsElement = (
      <div className="mc-card-props">
        {/* Cost removed from Characteristics display (kept in header) */}
        {t === 'ally' && (
          <>
            <div>
  ...
  // Minion / Villain / Leader / Player Minion
